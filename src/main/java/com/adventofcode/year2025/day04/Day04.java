    ...
    for (int i = currentRow; i < matrixHeight; i++) {
      for (int j = currentCol; j < matrixLength; j++) {
        if (input.get(i).charAt(j) == paperBale) {
          ...
          // Up-Right
          if (i > 0 && j < matrixLength - 1 && input.get(i - 1).charAt(j + 1) == paperBale)
            currentBaleCount++;
          // Down-Left
          if (i < matrixHeight - 1 && j > 0 && input.get(i + 1).charAt(j - 1) == paperBale)
            currentBaleCount++;
          // Down-Right
          if (i < matrixHeight - 1
              && j < matrixLength - 1
              && input.get(i + 1).charAt(j + 1) == paperBale) currentBaleCount++;
      ...
      for (int i = 0; i < currentGrid.size(); i++) {
        ...
        for (int j = 0; j < currentGrid.get(i).length(); j++) {
          if (currentGrid.get(i).charAt(j) == paperBale) {
            ...
            if (i < currentGrid.size() - 1 && currentGrid.get(i + 1).charAt(j) == paperBale)
              currentBaleCount++;
            if (j > 0 && currentGrid.get(i).charAt(j - 1) == paperBale) currentBaleCount++;
            if (j < currentGrid.get(i).length() - 1
                && currentGrid.get(i).charAt(j + 1) == paperBale) currentBaleCount++;
            if (i > 0 && j > 0 && currentGrid.get(i - 1).charAt(j - 1) == paperBale)
              currentBaleCount++;
            if (i > 0
                && j < currentGrid.get(i).length() - 1
                && currentGrid.get(i - 1).charAt(j + 1) == paperBale) currentBaleCount++;
            if (i < currentGrid.size() - 1
                && j > 0
                && currentGrid.get(i + 1).charAt(j - 1) == paperBale) currentBaleCount++;
            if (i < currentGrid.size() - 1
                && j < currentGrid.get(i).length() - 1
